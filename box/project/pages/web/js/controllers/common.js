...
//维修类别下拉框数据初始化
function getMaintenanceCategorySelectData($scope, $http, $compile, $location, $filter){
    if ($scope.ifOpenApi) {
        $scope.selectUrl = "getMaintenanceCateaorySelectList";
        $scope.selectOptName = "获取维修类别信息";
        $scope.selectPara = "maintenanceCategorySelectData";
        return $scope.getSelectInfoApi($scope);
    } else {
        $scope.maintenanceCategorySelectData = [{
            name: '下水疏通类',
            maintenanceType: {
                name: '水',
                id: 1
