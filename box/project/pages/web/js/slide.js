...
/*scroll*/
	...
	$("#slide1").find(".slideinner").find("a").mouseover(function(){
		if(_preNum != ($("#slide1").find(".slideinner").find("a").index(this))){
			...
			$("#special_mainimg").find("img").eq(_preNum).fadeIn(500);
		}
	})
...
mainFUNCTION
--------------*/
function funcbannerSlide(){
	
	$("#bannerSlide").find(".main").css({
		"position":"absolute"
	});
	for(var i = 0 ; i < $("#bannerSlide").find(".element").length ;i++){
		if($("#bannerSlide").find(".element").eq(i).find("img").attr("src")){
			_imgArray.push($("#bannerSlide").find(".element").eq(i).find("img").attr("src"));
		}
	}
	if(/*@cc_on!@*/false){
		//IE
	...
	function startslide(){
		$("#bannerSlide").find(".element").css("display","inline-block");
		
		$("#bannerSlide").find(".right").hide();
		$("#bannerSlide").find(".left").hide();
		
		$("#bannerSlide").find(".right").fadeIn(600);
		$("#bannerSlide").find(".left").fadeIn(600);
		
		$("#bannerSlide").find(".left,.right").VogueRollOver();
		
/*
		if($("#bannerSlide").find(".pict").length == 1){
			$("#bannerSlide").prepend($("#bannerSlide").find(".pict").clone());
			$("#bannerSlide").prepend($("#bannerSlide").find(".pict").clone());
			$("#bannerSlide").prepend($("#bannerSlide").find(".pict").clone());
		}else if($("#bannerSlide").find(".pict").length < 4){
			$("#bannerSlide").prepend($("#bannerSlide").find(".pict").clone());
		}
*/

		_maxpage = $("#bannerSlide").find(".pict").length;
		
		for(var i = 0 ; i < _maxpage ; i++){
			var _pos = Math.round(1002*(i-_currentpage)+$(window).width()/2-501);
			var _opa = 0.3;
			...
			$("#bannerSlide").find(".pict").eq(i).css({
				left:_pos,
				opacity:0
			})
			.animate({
		...
		$("#bannerSlide").stop().find(".main").removeClass("main");
		$("#bannerSlide").stop().find(".pict").eq(_currentpage).addClass("main").css({"position":"absolute"});
		...
		window.onresize = function(){
			for(var i = 0 ; i < _maxpage ; i++){
				...
				$("#bannerSlide").stop().find(".pict").eq(i).css({
					left:_pos,
					opacity:_opa
				})
			}
		}
		$("#bannerSlide").find(".right").click(nextPage);
		$("#bannerSlide").find(".left").click(prevPage);
	}
	...
	function prevPage(){
		...
		for(var i = 0 ; i < _maxpage ; i++){
			...
			if(_pos < -1002){
				_pos += _maxpage*1002
			}else if(_pos > $(window).width()+1002){
				_pos -= _maxpage*1002
			}
			$("#bannerSlide").find(".pict").eq(i)
			.stop()
			.css({
				left:_pos-1002
			})
...
document.write('<style type="text/css">div#bannerSlide div.main{display:none;}div#bannerSlide div.element{position:absolute !important;}</style>')
