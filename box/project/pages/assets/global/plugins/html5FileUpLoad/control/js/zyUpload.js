/* www.jq22.com */
(function ($, undefined) {
    $.fn.zyUpload = function (options, param, scope, timeout) {
        var scope = scope;
        ...
        return this.each(function () {
            ...
            this.init = function () {
                ...
                // 增加提示框
                $('.upload_main').after('<div class="alert alert-success" style="margin: 20px 0px 10px 0px;font-size:18px" ' +
                    'role="alert"><strong>提示!</strong> 您还可以上传 <span id="remainingAmount">'+scope.maxImg+'</span> 张图片</div>');
            };
            ...
            this.createHtml = function () {
                ...
                if (para.dragDrop) {
                    ...
                    html += '				<div class="convent_choice" style="width: 49%;">';
                    html += '					<div class="andArea">';
                    html += '						<div class="filePicker">选择文件</div>';
                    ...
                    html += '				<div class="btns">';
                    html += '					<div class="hide webuploader_pick">继续选择</div>';
            ...
            this.funFilterEligibleFile = function (files) {
                if (files.length > scope.maxImg) {
                    sweetAlert("警告", '一次性上选择最多"' + scope.maxImg + '"张图片', "warning");
                    return [];
                }
                else {
                    ...
                    for (var i = 0, file; file = files[i]; i++) {
                        try {
                            ...
                            if (scope.uploadTypeOnlyImg === true && ['jpg', 'png', 'jpeg', 'bmp', 'gif'].indexOf(file.type.toLowerCase().split('/')[1]) === -1) {
                                sweetAlert("警告", '请选择图片类型进行上传', "warning");
                            }
                            else if (file.size >= 51200000) { //5M
                                sweetAlert("警告", '您这个"' + file.name + '"文件大小过大,请小于5M再进行上传', "warning");
                            }
                            else if (ZYFILE.uploadFile.length + files.length <= scope.maxImg && length+ files.length <= scope.maxImg) {
                                // 在这里需要判断当前所有文件中
                                arrFiles.push(file);
                                // scope.thisPageAddImgCount = scope.thisPageAddImgCount + 1;
                            }
                            else {
                                sweetAlert("警告", '一次发布信息最多上传"' + scope.maxImg + '"张图片', "warning");
                            }
                        } catch (error) {
                            sweetAlert("警告", '选择文件失败,请重试!', "error");
                        }
                    }
                    return arrFiles;
                }
                ...
                var params = {
                    ...
                    onDelete: function (file, files) {
                        timeout(function () {
                            scope.$apply(function () {
                                scope.thisPageAddImgCount++;
                            })
                        }, 0);
                        // 移除效果
                        scope.uploadSuccessImgUrl[file.index] = null;
                        $("#uploadList_" + file.index).fadeOut();
                        $('#uploadInf>img#' + file.index).remove();
                        var tempUrl = scope.unEmptyList(scope.uploadSuccessImgUrl);
                        var length = tempUrl.length;
                        $('#remainingAmount').text(Math.min(scope.maxImg - length, scope.maxImg));
                        // 重新设置统计栏信息
                    ...
                    onSuccess: function (file, response) {
                        ...
                        if (response && response.message === 'SUCCESS') {
                            // 把所有上传成功的图片路径放入
                            scope.uploadSuccessImgUrl[file.index] = response.url;
                            // $("#uploadInf").append("<img id='" + file.index + "' style='width: 100%;height: auto' src='" + response.url + "'>");
                            var tempUrl = scope.unEmptyList(scope.uploadSuccessImgUrl);
                            var length = tempUrl.length;
                            $('#remainingAmount').text(Math.max(scope.maxImg - length, 0));
                            // 根据配置参数确定隐不隐藏上传成功的文件
                            if (para.finishDel) {
                                // 移除效果
                                $("#uploadList_" + file.index).fadeOut();
            ...
            this.addEvent = function () {
                ...
                $(".upload_btn").bind("click", function (e) {
                    ...
                    var tempUrl = scope.unEmptyList(scope.uploadSuccessImgUrl);
                    var length = tempUrl.length;
                    if (length >= 0 && length < scope.maxImg) {
                        $("#fileSubmit").click();
                    }
                    else if (length >= scope.maxImg) {
                        sweetAlert("警告", '一次发布信息最多上传"' + scope.maxImg + '"张图片', "warning");
                    }
                    // else {
                    //     sweetAlert("警告", "请先选中文件再点击上传", "warning");
                    // }
                });
