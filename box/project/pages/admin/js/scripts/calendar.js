...
var calendar = function () {
            ...
            .fullCalendar(
                {
                    ...
                    view: function() {
                        return {
                            start: $.fullCalendar.moment(new Date().Format('yyyy') + '-01-01T00:00:00'),
                            end: $.fullCalendar.moment(new Date().Format('yyyy') + '-12-31T00:00:00'),
                        }
                    },
                    weekends: true,
                    selectable: true,
                    ...
                    events: function (start, end, timezone, callback) {
                        ...
                        if(_year !== thisYear) {
                            if(_year >>0 > thisYear >>0) {
                                this.gotoDate(new Date(thisYear + "-12-01"));
                            }
                            else {
                                this.gotoDate(new Date(thisYear + "-01-01"));
                            }
                            return;
                        }
                        // 浏览器缓存中的日期集合,使用逗号分割
                    ...
                    eventClick: function(event, jsEvent, view ) {
                        ...
                        var _thisDay = $('td[data-date="' + time + '"].fc-widget-content');
                        thisDayClickEvent($(_thisDay), scope);
                    }
                });
    };
    ...
    var initFalseDateAndIcon = function (date, flag) {
        ...
        if (!flag) {
            ...
            if(_td.hasClass('fc-today')) {
                _td.removeClass('fc-today');
            }
        }
    };
    // 根据年月获取当前所有的日期列表
    ...
    var thisDayClickEvent = function (_this, scope) {
        // 当前状态为未记录日历情况
        if (_this.hasClass('calendarTableFalse')) {
            _this.removeClass('calendarTableFalse');
            _this.children('img.calendarTipImg').remove();
            _this.append(trueImgStr);
            getCheckedDate(scope);
        }
        else if (!_this.hasClass('calendarTableDisabled')) {
            // 不是今天框情况，简单处理
            if (!_this.hasClass('fc-today')) {
                _this.addClass('calendarTableFalse');
            }
            // 今天框 删除掉今天 class,添加上false的class
            else {
                _this.removeClass('fc-today').addClass('calendarTableFalse');
            }
            _this.append(falseImgStr);
            getCheckedDate(scope);
        }
    };
